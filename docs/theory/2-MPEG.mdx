...
As we explained in the previous lesson, standarization is usually developed by groups of experts in the subject. In this case, the Moving Picture Experts Group (MPEG) was established by ISO in 1988 and created the standard (named of course **MPEG-1**) in 1993.

MPEG-1 is a standard fomat for compressing and storing digital audio & video. This is a lossy compression method, but unlike images, all video compression formats are lossy, at least for now. 

It was designed to compress VHS-quality raw digital video and CD audio down to 1.5 Mbit/s ( 26:1 and 6:! compression ratios respectively). VHS had a resolution of only 360x240 pixels, which now would be low quality. It also allowed to create the Video CD format, which didn't succeed. 

The conceptual idea behind the MPEG-1 was to create a smart encoder which run on specialized hardware, and then a simple decoder that could be used by cheap hardware at homes.
...
In the previous lesson we saw how JPEG could compress an image (or frame). The MPEG doesn't work with each frame, instead uses an interframe or temporal compression. The following diagram describes the method's workflow:
...
We can see there is a lot of repetition within the frames: the blue background or the different still objects. We can devide the frames in three categories:

#### I Frame (intraframe, keyframe)

Which is a self-contained frame. It doesn't rely on anything to be rendered, looking similar to an static photo. The frist frame is usually an I-frame but we can see them inserted regularly in between of other types of frames.
...
The frame types are used to provide better compression. When using this technique, MPEG sequences and reorders the frames with the pattern IBBBPBBBP. An I frame is the most 'expensive' frame, followed by a P frame and a B frame. Therefore, we can obtain a very convincing and compressed result by only using part of the video's information.
...
Motion estimation examines the movement of objects in an image sequence and tries to obtain vectors representing the estimated motion. In this part of the encoding process, we will generate the motion vector data to then be consumed by the decorder.

> Note: motion estimation and motion compensation is considered as a single block in some resources, but are two different processes. Motion compensation is the use of the motion estimation to achieve compression. If you can describe the motion, then you have to describe the changes that occur after compensating for that motion.

The following sections will be explained without a lot of depth into the formulas or mathematical operations behind them, if you want to learn more check the full slides in PDF (ADD LINK).
...
Once we have the motion vector for each block, we perform the **block matching** where we apply the vector to the original block and make it match with the next frame, for every block. As before, further information can be found in the slide PDF.
...
MPEG-1 has its own codec based on psychoacustics. MPEG-1 Layer I (.mp1) is defined in ISO/IEC 11172-3, which first version was published in 1993. It supports the following sampling rates and bitrates:
...
- Improved version of the audio codec:
  ...
  - Improved sampling and bit rates: 16000, 22050, 24000Hz and 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144 and 160 kbit/s, respectively. Sampling rates are the half of those in MPEG-1, and were added to mantain higher quality sound when encoding audio at lower bitrates.
