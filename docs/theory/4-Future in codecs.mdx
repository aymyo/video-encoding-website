...
speeds were achieved. It was then cheaper and easier to access fiber connections depcrecating the
old modems and letting users connect to high speeds through an ethernet cable.

In this context, video over IP (IPTV) started to roll out. IPTV (Internet Protocol TeleVision) is a
wide concept to express all video delivered through the internet. As the Moore's law dictates,
encoding processing was way faster than before, allowing to do in real-time encoding that was
previouly impossible. In this environment, some companies related to video started to grow, such as:

- **Netflix**: starting out as an ordinary mail movie rental service, competitor of _Blockbuster_.
  They started to do data-mining and big data to make individual film documentations through their
  website, and allowed to order the DVDs from there. The competition was hard, and just before
  bankrupcy in 2005, they started sharing online its content as a last resort strategy, which worked
  very well.

- **Youtube**: created by 3 former Paypal employees in February 2005, aimed to be an enhancment for
  blogs and social networks, a service to upload content and insert it into user’s website. Myspace
  users started adopt this service in their comments and profiles, increasing its popularity. Before
  _Youtube_, when a company wanted to have videos in their website, they needed to take care of
  encoding, storage, delivery, web player, etc. When _Nike_ launched a Ronaldinho ad in late 2005
  using _Youtube_, other companies started to focus on it, and finally, on October 2006 Google
  bought it for 1650M$.

### Current situation

The two companies listed before had been for years the biggest promoters of video through the
internet for commercial and non-commercial use. However, over the past few years, specially after
the covid pandemic, a lot of video on demand companies have appeard to share the pie:

![useVoDuse](https://user-images.githubusercontent.com/40371955/147357779-ba812548-089e-4d28-a826-e52cb8aa4dd9.png).

In fact, video traffic through the internet has increased exponentially, representing now an
estimated **70% - 80% of internet traffic**.

This situation changed the aproach towards future video codecs, which as we've seen, are always
created to solve technological challenges. This are the technological challenges of today, focused
on IPTV:
...
started the _codec wars_. Companies started to explore royalty-free codecs to reduce costs (the same
way _Apple_ started to use their own audio codec to avoid paying royalties to MP3). This generated a
competition between codecs in order to become the standard. This is used by companies as a weapon
against competitors, making better or worse support of codecs in their hardware or browsers.

![codeccomparison](https://user-images.githubusercontent.com/40371955/147358488-3a259d57-71ad-47b5-afad-0dcd1ae4fd6a.png)

A summary of this technological-economic wars is: If you get a patent, you get paid. If you develop
...
the cinema industry forced the Betamax to become obsolete.
...
the VP8 format, and released a specification of the format under the Creative Commons Attribution
3.0 license. That same year, Google also released libvpx, the reference implementation of VP8, under
...
As you can see, all video codecs are based on the same concept, but with little differences. VP8 is
very similar to h264, it changes the prediction modes (motion compensation) and uses DCT with 4x4
macroblocks and Hardamard transform.

VP8 only supports progressive scan video signals with 4:2:0 chroma subsampling and 8 bits per
sample. Macro blocks can comprise 4×4, 8×8, or 16×16 samples and motion vectors have quarter-pixel
precission.
...
The VP8 also added its own kind of file, the **Matroska** `.mkv`. The Matroska Multimedia Container
is a free, open-standard container format, a file format that can hold an unlimited number of video,
audio, picture, or subtitle tracks in one file. Matroska is similar in concept to other containers
like AVI, MP4, or Advanced Systems Format (ASF), but is entirely open in specification, with
implementations consisting mostly of open source software.
...
Image Format (HEIF) is based on HEVC. As of 2019, HEVC is used by 43% of video developers, and is
the second most widely used video coding format after .h264. It has a basic free version; but it’s
patented and theorically you should pay if you use .mp4 files.
...
It also uses a new version of CABAC, and itroduces new modes for motion estimation (33 vector
...
The most important aspect about the codec is its huge reduction of the size of the files compared to
h264, and the mantainment of video quality. The estimated ratio is between -25% and -50% bitrate
...
support for the ongoing development of the successor AV1, of which Google took part, led to growing
...
The company grants free usage of its own related patents based on reciprocity, i.e. as long as the
user does not engage in patent litigations.
...
Another peculiarity is that trasnform coefficients are scanned in a round pattern (increasing
distance from the corners):
![vp9pattern](https://user-images.githubusercontent.com/40371955/147360735-1e788602-ab42-4cf1-8e71-fb0574b7230b.png)

### WebM

As the previous codec, it has its own video file `.WEBM`. WebM is an audiovisual media file format,
which is primarily intended to offer a royaltyfree alternative to use in the HTML5 video and the
...
I/O conference. Didn’t had support on Microsoft Edge/Explorer until 2017 and its still not supported
on Safari (_you already know what to say here_).
